...
class EDSR(object):
	...
	def __init__(self,img_size=32,num_layers=32,feature_size=256,scale=2,output_channels=3,sc_factor=0.5):
		...
		target_mean = 127
		#target_mean = tf.reduce_mean(self.target, 2, keep_dims = True)
		#target_mean = tf.reduce_mean(self.target, 1, keep_dims = True)

		image_input = x - input_mean
		image_target = y - target_mean
		'''
